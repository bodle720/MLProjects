...
def summarize_buckets(s3_client):
    """
    Summarize all S3 buckets in the account, including key attributes.

    Parameters
    - s3_client: The S3 client object for the desired region.
...
def delete_s3_bucket(s3_resource, bucket_name, force=False):
    """
    Delete an S3 bucket. Optionally delete all objects (and versions) before removal.

    Parameters:
    - s3_resource: The S3 resource object for the desired region.
...
def upload_local_file_to_s3(s3_client, local_path, bucket_name, object_name = None):
    """
...
def download_s3_obj_to_local_file(s3_client, bucket_name, object_key, local_path):
    """
...
def upload_dict_to_s3(s3_client, data, bucket_name, object_key):
    """
...
def upload_text_to_s3(s3_client, text_body, bucket_name, object_key):
    """
...
def upload_dataframe_to_s3(s3_client, df, bucket_name, object_key, encoding='utf-8', index=False):
    """
...
def get_dict_from_s3(s3_client, bucket_name, object_key, encoding='utf-8'):
    """
...
def get_text_from_s3(s3_client, bucket_name, object_key, encoding='utf-8'):
    """
...
def get_dataframe_from_s3(s3_client, bucket_name, object_key, encoding='utf-8', delimiter=','):
    """
...
def upload_df_or_dict_as_parquet_to_s3(s3_client, data, bucket_name, object_key, engine='pyarrow', compression='snappy', index=False, max_level=2):
    """
...
def get_df_or_dict_parquet_from_s3(s3_client, bucket_name, object_key, return_as_dict=False, engine='pyarrow'):
    """
