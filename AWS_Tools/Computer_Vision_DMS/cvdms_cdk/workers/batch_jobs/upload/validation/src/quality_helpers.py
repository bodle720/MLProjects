'''
Here’s a human-readable cheat sheet for each field (assuming we compute luma in **0–255** and fractions in **0–1**):

### `luma_mean`

Average brightness of the image (computed from grayscale/luminance). Range ≈ **0–255**; lower = darker (night/underexposed), higher = brighter (sunny/overexposed).

### `luma_p10`

10th percentile brightness — a “dark baseline” for the image. Range **0–255**; low values mean lots of very dark pixels/shadows.

### `luma_p90`

90th percentile brightness — a “bright baseline” for the image. Range **0–255**; high values mean lots of very bright pixels/highlights/glare.

### `dark_frac`

Fraction of pixels considered “dark” (below a fixed luma threshold, e.g. < 30). Range **0.0–1.0**; higher means more of the image is in deep shadow/low-light.

### `bright_frac`

Fraction of pixels considered “bright” (above a fixed luma threshold, e.g. > 225). Range **0.0–1.0**; higher means more blown highlights/glare/headlights/overexposure.

### `contrast_luma_std`

Standard deviation of luma values — a global contrast measure. Range ≈ **0–127+** (practically 0–100ish); low = flat/low-contrast (foggy/overcast), high = strong contrast (sharp shadows, vivid lighting).

### `contrast_luma_p90_p10`

Difference between bright and dark percentiles (`luma_p90 - luma_p10`), another robust contrast/dynamic-range measure. Range **0–255**; higher = wider brightness spread, lower = compressed/flat lighting.

### `blur_laplacian_var`

Variance of a Laplacian edge filter on grayscale — a “sharpness” proxy. Range is **unbounded** but typically ~**0–1000+** depending on resolution/downsampling; lower = blurrier (motion/defocus), higher = sharper edges.

### `sat_mean`

Average color saturation (from HSV S channel). Range **0.0–1.0**; low = gray/washed-out (fog/snow/overcast), high = vivid colors (sunny scenes, strong chroma).

### `colorfulness`

Hasler–Süsstrunk colorfulness metric combining chroma spread and bias. Range is **unbounded** but typically ~**0–100+**; low = dull/monochrome scenes, high = very colorful scenes.

### `lighting_bucket`

Categorical lighting condition derived from luma stats and dark/bright fractions. One of **night, low_light, normal, bright, glare**; use it for slicing performance by illumination.

### `blur_bucket`

Categorical sharpness derived from `blur_laplacian_var`. One of **sharp, mild_blur, blurry**; helpful for seeing how motion/defocus affects detection.

### `contrast_bucket`

Categorical contrast derived from `contrast_luma_std`. One of **low, medium, high**; good for analyzing fog/flat lighting vs normal scenes.

### `color_bucket`

Categorical color richness derived from `sat_mean` and `colorfulness` (for grayscale images, defaults to low). One of **low, medium, high**; useful for separating washed-out vs vivid scenes.
'''
