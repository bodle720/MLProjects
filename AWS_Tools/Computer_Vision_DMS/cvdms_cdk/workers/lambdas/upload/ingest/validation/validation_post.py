...
def handler(event, context):
    ...
    log(
        ...
        f"{TASK_NAME} Starting post-ingest verify for job {job_id} (expected_count={original_count})"
    )
    ...
    log(
        ...
        event_type,
        LOG_FIREHOSE_STREAM_NAME,
        f"{TASK_NAME} Validation ingest complete for job {job_id}: expected_count={original_count}, new_count={new_count}"
    )
