    ...
    Expected Step Functions per-item input (from item_selector):
      {
        ...
        shard, rows_read,
        upload_key,
        imagery_key (None for dedup),
        labels_key (None for dedup)
      }
    """
    try:
        job_id = event["job_id"]
        ...
        raise RuntimeError(f"[DEDUP_INGEST_MAP] Missing key: {e}, event={json.dumps(event)}")

    if not job_id or job_id == "unknown":
        raise RuntimeError("[DEDUP_INGEST_MAP] missing job_id")
    if not shard:
        raise RuntimeError("[DEDUP_INGEST_MAP] missing shard")
    if not upload_key:
        raise RuntimeError("[DEDUP_INGEST_MAP] missing upload_key")
