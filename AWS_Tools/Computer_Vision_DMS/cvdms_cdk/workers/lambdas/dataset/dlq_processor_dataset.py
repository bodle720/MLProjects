...
def handler(event, context):
    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            log(job_id, user, event_type, LOG_FIREHOSE_STREAM_NAME, f"{TASK_NAME} Temp S3 cleanup failed: {str(e)}", level="error")
        ...
        if not release_success:
            if str(release_msg).startswith("lock_not_held_by_job_id:"):
                log(job_id, user, event_type, LOG_FIREHOSE_STREAM_NAME,
                    f"{TASK_NAME} Lock is not held by this job: {release_msg}",
                    level="warning")
            else:
                log(job_id, user, event_type, LOG_FIREHOSE_STREAM_NAME,
