...
def metric_value_from_results(
    *,
    ...
    Examples:
        monitor='val_f1_macro'
        monitor='val_macro_average_precision'
        monitor='val_hamming_accuracy'
        monitor='train_loss'
    """
...
def epoch_metrics_dict(
    *,
    ...
    For backward convenience, this also writes:

        {split}_accuracy

    as an alias for hamming accuracy. In multi-label classification this is not
    argmax accuracy; it is per-label decision accuracy.
    """
    metrics = {
        f"{split}_loss": float(loss),
        f"{split}_accuracy": float(hamming_accuracy),
        f"{split}_hamming_accuracy": float(hamming_accuracy),
        f"{split}_hamming_loss": float(hamming_loss),
        f"{split}_subset_accuracy": float(subset_accuracy),
        f"{split}_precision_micro": float(precision_micro),
        f"{split}_recall_micro": float(recall_micro),
        f"{split}_f1_micro": float(f1_micro),
    ...
    if macro_average_precision is not None:
        metrics[f"{split}_macro_average_precision"] = float(macro_average_precision)

    if micro_average_precision is not None:
        metrics[f"{split}_micro_average_precision"] = float(micro_average_precision)
