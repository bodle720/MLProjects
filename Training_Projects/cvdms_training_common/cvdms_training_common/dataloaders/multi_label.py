"""
DataLoader builders for CVDMS multi-label training projects.

This module provides convenience helpers for turning CVDMS dataset-version
metadata into PyTorch DataLoaders for multi-label classification.
...
from cvdms_training_common.datasets.multi_label import (
    CvdmsMultiLabelDataset,
    build_default_multi_label_transforms,
    load_multi_label_dataset_for_split,
)
...
from cvdms_training_common.dataloaders.common_utils import (
    build_dataloader,
...
class MultiLabelDataBundle:
    """
    Container for multi-label datasets and DataLoaders.
    ...
    train_dataset: CvdmsMultiLabelDataset
    val_dataset: CvdmsMultiLabelDataset
    test_dataset: CvdmsMultiLabelDataset
...
def build_multi_label_data_bundle(
    *,
    metadata_uri: str,
    batch_size: int,
    num_workers: int = 0,
...
) -> MultiLabelDataBundle:
    """
    Build train/val/test datasets and DataLoaders for multi-label classification.
    ...
    if metadata.label_type != "multi-label":
        raise ValueError(
            "build_multi_label_data_bundle requires metadata.label_type='multi-label', "
            f"got {metadata.label_type!r}"
        )

    resolved_train_transform = train_transform or build_default_multi_label_transforms(
        image_size=image_size,
        train=True,
    )
    resolved_eval_transform = eval_transform or build_default_multi_label_transforms(
        image_size=image_size,
        train=False,
    )
    ...
    train_dataset = load_multi_label_dataset_for_split(
        metadata=metadata,
        split="train",
    ...
    val_dataset = load_multi_label_dataset_for_split(
        metadata=metadata,
        split="val",
    ...
    test_dataset = load_multi_label_dataset_for_split(
        metadata=metadata,
        split="test",
        transform=resolved_eval_transform,
        image_loader=resolved_image_loader,
